...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json(
      { error: { code: "SERVER_ERROR", message: err.message || "Failed to list institutions" }, requestId },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: { code: "SERVER_ERROR", message: err.message || "Failed to create institution" }, requestId },
      { status: 500 }
    );
  }
}
