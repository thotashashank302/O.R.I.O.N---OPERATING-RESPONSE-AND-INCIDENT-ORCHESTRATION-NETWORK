...
async function loadIncident(incidentId: string, context: UserContextItem): Promise<IncidentDetails> {
  ...
  if (!response.ok || !payload.data?.incident) {
    throw new Error(payload.error?.message || 'Failed to load incident');
  }
...
export default function IncidentDetailPage({
  ...
  useEffect(() => {
    ...
    loadIncident(incidentId, activeContext)
      ...
      .catch((error: unknown) => {
        if (!cancelled) {
          setErrorMsg(error instanceof Error ? error.message : 'Error loading details');
        }
      })
      .finally(() => {
