...
export default async function HODPage() {
  ...
  const pendingApprovals: PendingApproval[] = (rawApprovals ?? []).map((a: any) => ({
    id: a.id,
  ...
  const incidentQueue: IncidentQueueItem[] = (rawIncidents ?? []).map((inc: any) => {
    ...
    return {
      ...
      deadline: null,
      is_overdue: ageMinutes > 20, // 20-min escalation threshold per policy §7
