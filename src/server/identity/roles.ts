...
export async function getUserContexts(userId: string): Promise<UserContextResponse | null> {
  ...
  const membershipRows = memberships as unknown as MembershipContextRow[];
  for (const m of membershipRows) {
    ...
    contexts.push({
      ...
      roles: ((grants || []) as unknown as ContextGrantRow[]).map((g) => ({
        role: g.role,
...
export async function verifyActiveMembershipAndRole(
  ...
  return {
    valid: true,
    membership: membership as VerifiedMembership,
    roles: activeRoles,
  };
}
