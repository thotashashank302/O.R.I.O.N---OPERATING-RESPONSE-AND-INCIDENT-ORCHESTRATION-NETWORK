...
test("renders the ORION entry experience", async ({ page }, testInfo) => {
  await page.goto("/");
...
test("login remains usable on a phone viewport", async ({ page }, testInfo) => {
...
test("secure acknowledgement GET is non-mutating and handles missing links", async ({ page }, testInfo) => {
  ...
  await page.screenshot({ path: testInfo.outputPath("email-action-desktop.png"), fullPage: true });
});

test("secure acknowledgement form remains usable on a phone viewport", async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 390, height: 844 });
  ...
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
  await page.screenshot({ path: testInfo.outputPath("email-action-mobile.png"), fullPage: true });
});
